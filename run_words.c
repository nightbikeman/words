...
//
	if (argc == 4)  {
		...
		if (argc != 3)  {printf("Run Benchmark: Usage -b iters <threads>"); exit(1);}
...
#pragma omp barrier
	...
	if (tid == 0) {
		...
		else	iterations=iterations/cores;
...
//		printf("Thread: %d - Testing test_words[%s] and test_words[%s] \n",tid,test_words[rand1],test_words[rand2]);
	...
	printf("The Benchmark took %f wall clock seconds to complete.\n\n", difftime(end, begin));
      ...
      case 'c':       // Create an o/p file
		if (argc != 4)  {printf("Create input file: Usage -c inum_lines words_file\n"); exit(1);}
...
// Create a new input words file comprising n lines of random alpha numeric strings
      ...
      case 'o':	 
		if (argc != 3)  {printf("Output into format: Usage -o input_file\n"); exit(1);}
...
//  Load the truths words file and write out into 3 formats
      ...
      case 's':        
		if (argc != 5)  {printf("Usage -s nth_order<1|2> entity1 entity2\n"); exit(1);}
