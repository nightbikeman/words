...
static void add_ent(struct entity **ents,long *no,char *name,struct hsearch_data *htab)
{
...
static long find_entity(char *word,struct hsearch_data *htab,long max)
{
        int i=max+1; 
        ENTRY ent;
        ENTRY *e=&ent;
...
//
    fd_entities_path = open(ENTITIES_FILEPATH, O_RDWR | O_CREAT | O_TRUNC, (mode_t)0600);
    if (fd_entities_path == -1) {
        perror("Error opening mmap file for writing");
        exit(EXIT_FAILURE);
    }
    fd_hash_path = open(HASH_FILEPATH, O_RDWR | O_CREAT | O_TRUNC, (mode_t)0600);
    if (fd_hash_path == -1) {
        perror("Error opening mmap file for writing");
        exit(EXIT_FAILURE);
    }
...
//
    entities = mmap(0, ENTITIES_FILESIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd_entities_path, 0);
    if (entities == MAP_FAILED) {
        close(fd_entities_path);
        perror("Error mmapping the entities file");
        exit(EXIT_FAILURE);
    }
    htab = mmap(0, HASH_FILESIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd_hash_path, 0);
    if (htab == MAP_FAILED) {
        close(fd_hash_path);
        perror("Error mmapping the entities file");
        exit(EXIT_FAILURE);
...
// End MMAP stuff
    ...
    if ( ! json) printf("Creating the in memory table... \n");
    ...
    while(1==fscanf(in, "%[^\n]%n\n", line, &line_length)){//read one line
        ...
        char *ptr;
        {
            ...
            if (i > num_entities ) {
                    ...
                    if ( ! json && DEBUG) printf("***At line %d Root Entity %s was already found \n",num_lines,root);
                    num_dups++;
        ...
        for(;word=strtok_r(NULL,",",&ptr);) 
        {
            ...
            if (sub_entity > num_entities ) {
                    ...
                    if ( ! json && DEBUG) printf("***At line %d Sub Entity %s was already found \n",num_lines,word);
                    num_dups++;
            }
...
// Now print out the entire set of Entities:
	...
	printf("The total number of Entities are %d read in %d lines with %d duplications.\n",num_entities,num_lines,num_dups);
    }
