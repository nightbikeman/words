...
static void error(char *msg,int code)
{
    fprintf(stderr,"code=%d:%s",msg,code);
    exit(code);
}
static void add_ent(redisContext *c,char * word)
{
     redisReply *reply = redisCommand(c, "SET %s \"\"", word);
     if ( reply == NULL ) error("database set failed",3);

     freeReplyObject(reply);
}

static void add_link(redisContext *c,char *key,char *word)
{
     redisReply *reply = redisCommand(c, "APPEND %s ,%s", key,word);
     if ( reply == NULL ) error("database append failed",3);

     freeReplyObject(reply);
}

static int find_entity(char *word,redisContext *c)
{
     redisReply *reply = redisCommand(c, "EXISTS %s", word);
     if ( reply == NULL ) error("database exists failed",2);
